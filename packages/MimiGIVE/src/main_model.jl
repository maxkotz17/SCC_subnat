...
function get_model(; Agriculture_gtap::String = "midDF",
    ...
    add_comp!(m, GlobalTempNorm, :TempNorm_2020, after = :TempNorm_1995to2005); #for subnational damages 

    # Add Ocean Heat Accumulator to Link FAIR and BRICK
    add_comp!(m, OceanHeatAccumulator, after = :TempNorm_2020);
    ...
    # Add subnational macroeconomic damages
    add_comp!(m, subnat_damages, :subnat_damages, first = damages_first, after = :hs_damage);
    ...
    #TempNorm_2020 - Normalize temperature to deviation from 2020 for subnational damage component
    update_param!(m, :TempNorm_2020, :norm_range_start, 2020)
    update_param!(m, :TempNorm_2020, :norm_range_end, 2020)
    connect_param!(m, :TempNorm_2020 => :global_temperature, :temperature => :T)
    ...
    # --------------------------------------------------------------------------
    #	Subnational macro-economic damages
    # --------------------------------------------------------------------------
    ...
    connect_param!(m, :DamageAggregator => :damage_subnat, :subnat_damages => :damages)
